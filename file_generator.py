...
- 0.3초마다 모든 CAM 폴더(날짜2 x CAM6 = 12곳)에 각각 PNG 1개씩 생성.
  파일명은 각 파일의 생성 시각(img_YYYYMMDD_HHMMSS_microsec.png).
- 30분 생성 -> 전체 폴더 비우기 -> 다시 30분 생성 ... 무한 반복(사이클).
- 시스템 + 프로세스(기본 file-agent) RAM 모니터링, ram_log.txt 로 기록.
- PNG 생성은 외부 라이브러리 불필요. 프로세스별 RAM 추적만 psutil 필요(선택).
- Ctrl+C 로 중지.

사용 예 (Windows):
    python file_generator.py
    python file_generator.py --base "경로1" "경로2"
    python file_generator.py --watch-process file-agent S3Agent
    python file_generator.py --log-file "D:\\test\\ram.txt"
    python file_generator.py --no-purge
...
def main() -> int:
    ...
    ap.add_argument("--mon-interval", type=float, default=5, help="RAM 출력/기록 간격(초), 기본 5")
